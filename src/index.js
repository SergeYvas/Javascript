...
const findAllPSiblings = where => {
    return [...where.children].filter(node => node.nextElementSibling && node.nextElementSibling.tagName === 'P')
};
...
function deleteTextNodes(where) {
    for (const node of where.childNodes) {
        node.nodeType === 3 && where.removeChild(node)
    }
...
function deleteTextNodesRecursive(where) {
    ...
    for (let node of nodes) {
        node.nodeType === 3 && where.removeChild(node);
        node.childNodes.length > 0 && deleteTextNodesRecursive(node);
    ...
    (function collector(node) {
        for (let item of node.childNodes) {
            const { 
                ...
                nodeType: type,
                classList: classes = [],
                childNodes: children } = item;
            const isTextNode = type ===3;
            const isTagInStat = tag in statistic.tags;
            ...
            if (classes.length) {
                for ( let data of classes) {
                    const isClassInStat = typeof data === 'string' && data in statistic.classes;

                    isClassInStat ? 
                        statistic.classes[data]++ 
                        : 
                        statistic.classes[data] = 1;
                
                }
            }
            children.length > 0 && collector(item);
        }
    }(root));
...
function observeChildNodes(where, fn) {
    ...
    return new MutationObserver(cb).observe(where, config);
}
